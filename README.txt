...
the search criteria. These entries are displayed in a table that lists the row,
trail name, description, status, phase, type, condition, intervention, age, sex,
dates, and location of each. If an invalid search term is given, an error page
is shown. Users have the option to further refine their search with additional
search options such as phase, date range, age range, sex, and type of trial.
Clicking into a particular trial brings the user to that trial page. Here, the
full description and information of the trial is shown. Users also now have the
option to save the clincial trial.
...
the database interface is simple already and it would be redundant to make
users undergo a subsequent screening just to access the database.

The results web page provides the most interesting database operations as it
combines various parameters and inputs from the search bar, toggle buttons, and
dropdown menu options. Using these, it constructs a query that spans multiple
data tables in order to return the relevant results that the user wants. This
page is the crux of the database and what users are there to see.
